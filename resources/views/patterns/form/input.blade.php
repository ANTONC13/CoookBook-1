<?php
    ...
    $placeholder     = $has_error || $type == 'password' ? '' : $field_description;
?>


@if( $type == 'hidden' )
    <input type="hidden" name="{{ $field_name }}" value="{{ $value }}">
    @if ( $has_error )<p class='red-text left'>{{ $data_error_text }}</p>@endif
@elseif( $type == 'file' )
    ...
    <div class="row">
        <div class="input-field col s12">
            ...
            @if ( $has_error )
                ...
                <p class='red-text left'>{{ $data_error_text }}</p>
            @else
